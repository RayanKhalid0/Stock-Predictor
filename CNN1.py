...
def model_builder(hp):
    ...
    for i in range(hp.Int('num_conv_layers', 1, 3)):
        model.add(Conv1D(filters=hp.Int(f'filters_{i}', min_value=32, max_value=256, step=32),
...
tuner = kt.RandomSearch(
    ...
    directory='tuner_results_CNN3',
    project_name='cnn_tuning_SPY_v3'
)

early_stopping = EarlyStopping(monitor='val_loss', patience=200, restore_best_weights=True)
tuner.search(X_train, y_train, epochs=100, validation_data=(X_test, y_test),
             batch_size=tuner.get_best_hyperparameters()[0].get('batch_size'), callbacks=[early_stopping])
...
history = model.fit(X_train, y_train, epochs=1000, validation_data=(X_test, y_test),
                    batch_size=best_hps.get('batch_size'), callbacks=[early_stopping])
model.save('CNN-SPY-Hyper-v2.h5')
